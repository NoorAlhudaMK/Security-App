...
import '../BLoC/visitor_check_out_bloc.dart';
import '../BLoC/visitor_check_out_event.dart';
import '../BLoC/visitor_check_out_state.dart';

class CheckOutQrScannerPage extends StatelessWidget {
  final int gateId;
  final AppColors appColors = AppColors();

  CheckOutQrScannerPage({Key? key, required this.gateId}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: BlocConsumer<VisitorCheckOutBloc, VisitorCheckOutState>(
          listener: (context, state) {
            if (state is VisitorCheckOutSuccessState) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text("تم تسجيل الخروج بنجاح للزائر: ${state.visitor.visitorName}"),
                  backgroundColor: Colors.green,
                ),
              );
              _scannerController.stop();
              Navigator.pop(context);
            } else if (state is VisitorCheckOutFailureState) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
          ...
          builder: (context, state) {
            final isLoading = state is VisitorCheckOutLoadingState;
                                  ...
                                  for (final barcode in barcodes) {
                                    if (barcode.rawValue != null && !isLoading) {
                                      ...
                                      if (token != null && context.mounted) {
                                        BlocProvider.of<VisitorCheckOutBloc>(context).add(
                                          SubmitVisitorCheckOutEvent(
                                            token: token,
                                            qrToken: code,
                                            gateId: gateId,
                                          ),
                        ...
                        builder: (context, active, child) {
                          return ElevatedButton.icon(
                            onPressed: () {
                              ...
                              isScanningActive.value ? _scannerController.start() : _scannerController.stop();
                            },
                            icon: const Icon(Icons.qr_code_scanner, color: Colors.white),
                            label: Text(
                              active ? "إيقاف الماسح" : "تشغيل الماسح",
  ...
  // تصميم الزوايا الأربع البرتقالية للماسح
